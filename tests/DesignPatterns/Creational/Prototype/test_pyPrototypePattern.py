...
from pyPantry.DesignPatterns.Creational.Prototype.PyPrototypePattern import PyPrototypePattern
...
class PyPrototypePatternTestCase(unittest.TestCase):
    ...
    def test_example(self):
        with patch('sys.stdout', new=StringIO()) as fake_out:
            ...
            self.assertIn("Original: Character(Name: Knight, Health: 100, Mana: 50)", output)
            self.assertIn("Cloned: Character(Name: Mage, Health: 80, Mana: 120)", output)
            self.assertIn("Unchanged Original: Character(Name: Knight, Health: 100, Mana: 50)", output)
