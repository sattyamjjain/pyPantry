...
class PyEventDrivenArchitecturePatternTestCase(unittest.TestCase):
    ...
    def test_example(self):
        with patch('builtins.print') as mocked_print:
            ...
            self.assertIn(call("Sending email notification for new user: {'id': 1, 'name': 'Alice'}"), output)
            self.assertIn(call("Product created: {'id': 1, 'name': 'Laptop'}"), output)
            self.assertIn(call("Order created: {'id': 1, 'user_id': 1, 'product_id': 1}"), output)
            self.assertIn(call("Sending email notification for new order: {'id': 1, 'user_id': 1, 'product_id': 1}"),
                          output)
