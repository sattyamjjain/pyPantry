...
class PyHalfSyncOrHalfAsyncPatternTestCase(unittest.TestCase):
    ...
    def test_sync_layer_process_request(self):
        ...
        with patch('builtins.print') as mocked_print:
            ...
            self.assertIn(call("Processing request: Test Request"), mocked_print.mock_calls)
            task = task_queue.get(timeout=1)
            task()
            self.assertIn(call("Handled request: Test Request"), mocked_print.mock_calls)
    ...
    def test_async_layer_run(self):
        ...
        with patch('builtins.print') as mocked_print:
            ...
            self.assertIn(call("Processing request: Test Request 1"), mocked_print.mock_calls)
            self.assertIn(call("Processing request: Test Request 2"), mocked_print.mock_calls)
            self.assertIn(call("Handled request: Test Request 1"), mocked_print.mock_calls)
            self.assertIn(call("Handled request: Test Request 2"), mocked_print.mock_calls)
